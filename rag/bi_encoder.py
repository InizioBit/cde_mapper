...
def parse_text(text):
    print(f"Paring Text for Embedding={text}")
    ...
    if ent_match:
        ...
        synonyms = (
            syn_match.group(1).split(";;") if syn_match and syn_match.group(1) else None
        )
        ...
        print(f"entity={entity}, synonyms={synonyms}, domain={parent_term}")
...
class MedCPT_Embeddings(Embeddings):
    def __init__(
        self, model_id: str = "ncbi/MedCPT-Query-Encoder", device: str = None, **kwargs
    ):
        """
        Initializes the Custom embedding class by loading the specified transformer model.
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        with torch.no_grad():
            ...
            if hasattr(outputs, "last_hidden_state"):
                ...
                raise ValueError(
                    "Model output does not contain last hidden state or hidden states"
                )
...
class SAPEmbeddings(Embeddings):
    ...
    def embed_documents_v1(self, texts: List[str]) -> List[List[float]]:
    ...
    def embed_documents(self, texts: List[str], type="docs") -> List[List[float]]:
        """Embed multiple documents by encoding entities and their synonyms together as per SAPBERT requirements."""
        embeddings = []
        batch_size = 128  # Batch size for inference

        # Process each text (each document)
        for text in texts:
            entity, synonyms, _ = parse_text(text)
            if type == "query":
                entity, synonyms, _ = text, None, None
            # Append entity and synonyms in one list
            all_names = [entity] + synonyms if synonyms else [entity]

            # Batch process embeddings
            all_embs = []
            for i in tqdm(
                range(0, len(all_names), batch_size),
                desc="Embedding Entities and Synonyms",
                unit="batch",
            ):
                # Tokenize in batches
                ...
                # Use CLS representation as embedding
                with torch.no_grad():
                    cls_rep = self.model(**toks_cuda)[0][
                        :, 0, :
                    ]  # CLS token representation (768-dimensional)
                all_embs.append(cls_rep.cpu().detach().numpy())

            # Concatenate embeddings from batches without any mean calculation
            all_embs = np.concatenate(all_embs, axis=0)

            final_embedding = (
                all_embs[0] if all_embs.shape[0] > 1 else all_embs.squeeze()
            )
            # print(
            #     f"shape of final_embedding: {final_embedding.shape}"
            # )  # should be (768,)
            final_embedding_list = final_embedding.tolist()
            # print(f"length of final_embedding_list: {len(final_embedding_list)}")
            embeddings.append(
                final_embedding_list
            )  # Store as list of lists for each document
    ...
    #     # Extract embeddings
    #     embeddings = self.feature_extractor(text)
    #     embdding = np.mean(embeddings, axis=0).mean(axis=0)
    #     return embdding

    def embed_query(self, text: str) -> List[float]:
        """Embed a single query using the same approach as for documents."""
        entity_embedding = self.embed_documents([text], type="query")[0]
...
def sparse_encoder(text: str) -> tuple[list[int], list[float]]:
    ...
    query_vec, _ = compute_vector(
        text=entity, tokenizer=tokenizer, model=model, device=device
    )
...
def compute_vector(text, tokenizer, model, device):
    ...
    tokens = tokenizer(
        text, return_tensors="pt", max_length=25, truncation=True
    )  # Ensure truncation

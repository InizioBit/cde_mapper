...
class AthenaFilters(BaseModel):
    conceptClass: list[str] | None = Field(
        default=None, example=["Lab Test", "Procedure"]
    )
    ...
    standard_concept: list[str] | None = Field(
        default=None, example=["Standard", "Classification", "Non-standard"]
    )
    ...
    pageSize: int = Field(
        default=15, ge=1, description="Number of results per page. Defaults to 5"
    )

    def __init__(self, **data):
        super().__init__(**data)
        self.conceptClass = (
            [self.validate_and_format(item) for item in self.conceptClass]
            if self.conceptClass
            else None
        )
        self.domain = (
            [self.validate_and_format(item) for item in self.domain]
            if self.domain
            else None
        )
        self.standard_concept = (
            [self.validate_standard_concept(item) for item in self.standard_concept]
            if self.standard_concept
            else None
        )
        self.vocabulary = (
            [self.validate_vocabulary(item) for item in self.vocabulary]
            if self.vocabulary
            else None
        )
    ...
    def validate_vocabulary(self, item: str) -> str:
        ...
        if item not in valid_vocabularies:
            raise ValueError(
                f"{item} is not valid; must be one of {valid_vocabularies}."
            )
        return item
...
def convert_to_documents(results) -> List[Document]:
    matching_documents = [
        Document(
            id=f"{str(item['vocabulary']).strip().lower()}:{str(item['code'])}",
            ...
            metadata={
                ...
                "parent_term": "",
                "concept_class": str(item["className"]).strip().lower()
                if item["className"]
                else None,
            },
        )
...
class RetrieverAthenaAPI(BaseRetriever):
    ...
    def __init__(self, filters: AthenaFilters, k: int = 15):
    ...
    async def _afetch_from_athena(self, query: str):
        ...
        try:
            ...
            if search_res:
                # print(f"length of athena search_res: {len(search_res)}")
                return search_res[: self.k]
            else:
                return []
    ...
    async def _aget_relevant_documents(
        ...
        try:
            ...
            matching_documents = [
                Document(
                    ...
                    metadata={
                        ...
                        "scode": item["code"],
                        "domain": str(item["domain"]).strip().lower(),
                        "is_standard": convert_standard_concept(
                            str(item["standardConcept"])
                        ),
                        "vocab": str(item["vocabulary"]).strip().lower(),
                        "parent_term": "",
                        "concept_class": str(item["className"]).strip().lower()
            ...
            print(f"Asynch length of athena documents: {len(matching_documents)}")
            return matching_documents
        except Exception as e:
            # display exact error message
            print(
                f"async Failed to fetch documents from Athena API: {e} for query: {query}"
            )
            return []
